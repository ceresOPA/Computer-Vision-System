...
@file_router.post("/photo", summary="上传图像")
async def upload_image(
    ...
    save_dir = "C:/Users/YL/Desktop/OldPhotoRectify/server/inputs"
    ...
    return {"imageUrl": f"http://127.0.0.1:81/api/inputs/{tmp_file_name}",
            "imageName": f"{tmp_file_name}",
            "appImgUrl":f"http://127.0.0.1:8080/inputs/{tmp_file_name}"}


@file_router.get("/colorize", summary="图像上色")
def check_image(imageName: str):
    server_dir = 'C:/Users/YL/Desktop/OldPhotoRectify/server/'
    print("imageName",imageName)
    os.system(f"python {server_dir}/colorization/colorize.py --input {server_dir}/inputs/{imageName}   --output {server_dir}/output  --use_gpu")
    return {"msg":"ok",
            "eccv16ImgUrl":f"/output/eccv16_{imageName}?random={random.randrange(1, 1000)}",
            "sigImgUrl":f"/output/sig_{imageName}?random={random.randrange(1, 1000)}"}


@file_router.get("/repair", summary="图像修复")
def check_image(imageName: str):
    server_dir = 'C:/Users/YL/Desktop/OldPhotoRectify/server'
    print("imageName",imageName)
    os.system(f"python {server_dir}/repairs/run.py --input_folder {server_dir}/inputs   --output_folder {server_dir}/output  --GPU 0 --with_scratch")
    return {"msg":"ok",
            "imageUrl":f"/output/final_output/{imageName.split('.')[0]+'.png'}?random={random.randrange(1, 1000)}"
            }
...
def check_image(model: str,imageName: str):
    ...
    print("model",model)
    print("imageName",imageName)
    if model=='mask-yolov5s' or model == 'mask-yolov5m':
